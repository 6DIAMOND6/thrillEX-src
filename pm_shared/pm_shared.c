...
int			g_iLadderFrame = 0;
float		g_flLadderSpeed = 0.0f;
qboolean	g_bLadderPunchSwap;
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	isAlpha = (atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "lm" ) ) >= 1) ? 1 : 0;
...
void PM_UpdateStepSound( void )
{
		...
		( speed >= velwalk || !pmove->flTimeStepSound ) )
	{
		...
		if (fLadder)
		{
			...
			//else
			//fvol = 0; //DarkKrysteq:
			// SERECKY JAN-17-26: Moved alphachecking method to
			// the PlayStepSound function instead.
...
void PM_WalkMove ()
{
	...
	if (pmove->flags & FL_IMMUNE_LAVA)	// this flag is set by a snapbug
		return;
...
void PM_Duck( void )
{
	...
	// fixed slowing down underwater since it was completely stupid
	if ( pmove->flags & FL_DUCKING && pmove->movetype != MOVETYPE_NOCLIP )
	{
		...
		{
			pmove->cmd.forwardmove *= PLAYER_DUCKING_MULTIPLIER*2;
			pmove->cmd.sidemove    *= PLAYER_DUCKING_MULTIPLIER*2;
			pmove->cmd.upmove      *= PLAYER_DUCKING_MULTIPLIER*2;
		}
	}
...
void PM_LadderMove( physent_t *pLadder )
{
	...
	if ( trace.fraction != 1.0 )
	{
		...
		float flSideSpeed = flSpeed * 0.6f;
		...
		if (!isAlpha)
		{
			g_flLadderSpeed = flSpeed;
			flSideSpeed = flSpeed;
		}

		if ( pmove->cmd.buttons & IN_BACK )
		{
			forward -= g_flLadderSpeed;
		}
		else if ( pmove->cmd.buttons & IN_FORWARD )
		{
			forward += g_flLadderSpeed;
		}
		else
		{
			forward = 0.0f;
			pmove->flNextPrimaryAttack = 300.0f;
		}
		...
		if ( pmove->cmd.buttons & ( IN_BACK | IN_FORWARD | IN_MOVELEFT | IN_MOVERIGHT ) )
		{
			if (g_flLadderSpeed >= MAX_CLIMB_SPEED)
				g_flLadderSpeed = 0.0f;
			g_flLadderSpeed += CLIMB_SPEED_DEC;
		}
		...
		{
			// I'm using flNextPrimaryAttack as a timer for alpha styled climbing,
			// since flNextPrimaryAttack is an unused var. in the pmove struct.
			// - serecky 1.14.26

			if ( pmove->flNextPrimaryAttack <= 0.0f && isAlpha )
			{
				pmove->flNextPrimaryAttack = 300.0f;
				g_flLadderSpeed = MAX_CLIMB_SPEED;

				switch ( pmove->RandomLong(0, 3) )
				{
				case 0:	pmove->PM_PlaySound( CHAN_VOICE, "player/pl_pain2.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM ); break;
				case 1: pmove->PM_PlaySound( CHAN_VOICE, "player/pl_pain4.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM ); break;
				case 2: pmove->PM_PlaySound( CHAN_VOICE, "player/pl_pain5.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM ); break;
				case 3: pmove->PM_PlaySound( CHAN_VOICE, "player/pl_pain6.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM ); break;
				}

				if ( g_bLadderPunchSwap )
				{
					pmove->punchangle[2] = CLIMB_PUNCH_Z;
					pmove->punchangle[0] = CLIMB_PUNCH_X;
					g_bLadderPunchSwap = false;
				}
				else
				{
					pmove->punchangle[2] = -CLIMB_PUNCH_Z;
					pmove->punchangle[0] = CLIMB_PUNCH_X;
					g_bLadderPunchSwap = true;
				}
				
				//if (pmove->runfuncs)
				//	pmove->PM_PlaybackEventFull(1, pmove->player_index, g_usLadderClimb, 0.0f, (float*)pmove->origin, (float*)pmove->origin, 0.0f, 0.0f, 0, 0, 0, 0 );
			}
...
void PM_CheckFalling( void )
{
	...
	{
		// putting this up here so later sounds can override this - serecky 1.3.26
		...
		pmove->punchangle[ 0 ] = pmove->flFallVelocity * 0.018;	// punch x axis
		pmove->punchangle[2] = pmove->flFallVelocity * 0.009;
		...
		// Vasia: This line was removed from later versions of SDK
		// Serecky: and i'll remove it AGAIN !!!!!!!! - Jan 15. 26
		//if ( pmove->RandomLong( 0, 1) )
		//{
		//	pmove->punchangle[2] = pmove->flFallVelocity * 0.009;
		//}
...
void PM_CheckParamters( void )
{
	...
	if ( pmove->flags & FL_FROZEN ||
		 ...
		 pmove->flags & FL_IMMUNE_LAVA ||		// this flag is set by a snapbug
		 pmove->dead )
	{
...
void PM_ReduceTimers( void )
{
	...
	if ( pmove->flNextPrimaryAttack > 0.0f )
	{
		pmove->flNextPrimaryAttack -= pmove->cmd.msec;
		if ( pmove->flNextPrimaryAttack < 0.0f )
		{
...
void PM_PlayerMove ( qboolean server )
{
	...
	if ( !pmove->dead && !(pmove->flags & FL_ONTRAIN) )
	{
		...
		if ( !PM_LadderMove_Alpha(pLadder) && pmove->movetype == MOVETYPE_FLY )
		{
	...
	//DarkKrysteq: In beta +use didnt slow you
