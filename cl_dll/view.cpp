...
void SCR_CalcRefdef(struct ref_params_s* pparams); // SERECKY 1-17-26: Stuff for H.W Mode viewsize commands.
...
// New CVars for stuff I find amusing
// -serecky 1.1.25
...
void V_CalcBlend(void)
{
	...
	v_blend[1] = g / 255.0;
	v_blend[2] = b / 255.0;
	v_blend[3] = a * cl_cshift_mult->value;
...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	int viewsize = IEngineStudio.IsHardware() ? scr_viewsize->value : pparams->viewsize;

	// SERECKY JAN-17-26: Update HUD S.W Viewsize
	if (!IEngineStudio.IsHardware())
	{
		if ((gHUD.m_iViewsize != pparams->viewsize) && (gHUD.m_iViewsize <= 120))
			gHUD.m_iViewsize = pparams->viewsize;
		//gEngfuncs.pfnConsolePrint(va("VIEWSIZE: %d\n", gHUD.m_iViewsize));
	}
	...
	// If cl_jitter_punch is on, round the punch axises
	// into integers to emulate how Quake does it...
	// - serecky 1.1.26

	if (cl_jitter_punch->value)
	{
		vec3_t jitter_punch[2]; // 0 == Server punch, 1 == Client punch

		for (i = 0; i < 3; i++)
		{
			jitter_punch[0][i] = (int)pparams->punchangle[i];
			jitter_punch[1][i] = (int)ev_punchangle[i];
		}

		// Add in the punchangle, if any
		VectorAdd ( pparams->viewangles, (float *)&jitter_punch[0], pparams->viewangles );

		// Include client side punch, too
		VectorAdd ( pparams->viewangles, (float *)&jitter_punch[1], pparams->viewangles);
	}
	else
	{
		// Add in the punchangle, if any
...
# ifdef _TFC	// TFC models override HL models
		...
		{ "models/p_squeak.mdl",		"models/v_chub.mdl"		},
		{ NULL, NULL } };
...
void SCR_SizeUp_f(void)
{
	if (!IEngineStudio.IsHardware())
	{
		// SERECKY JAN-17-26: Added new sizeup logic to account for
		// whether or not players would like to hide the certian parts
		// of the HUD.

		if (gHUD.m_iViewsize >= 120)
			gHUD.m_iViewsize = min(gHUD.m_iViewsize + 10, 150);
		else
	...
	gHUD.m_iViewsize = (int)gEngfuncs.pfnGetCvarFloat("scr_viewsize");
}
...
void SCR_SizeDown_f(void)
{
	if (!IEngineStudio.IsHardware())
	{
		// SERECKY JAN-17-26: Added new sizedown logic to account for
		// whether or not players would like to hide the certian parts
		// of the HUD.
