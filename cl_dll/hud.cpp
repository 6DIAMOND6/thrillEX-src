...
// putting these out here coz i can't be
// assed to properly like put these into the CHUD
// class - serecky 1.3.25
cvar_t *cl_flashlight_style;
cvar_t *cl_mp5_animskip;
cvar_t *original_rof = NULL;
cvar_t *hud_weaponhinter;
...
int __MsgFunc_RateOfFire(const char *pszName, int iSize, void *pbuf)
{
...
void CHud :: Init( void )
{
	...
	cl_flashlight_style = gEngfuncs.pfnRegisterVariable ( "cl_flashlight_style", "0", FCVAR_ARCHIVE );
...
//	m_hsprFont = LoadSprite("sprites/%d_font.spr");
	...
	if ( !m_pSpriteList )
	{
		...
		if (m_pSpriteList)
		{
			...
			for ( j = 0; j < m_iSpriteCountAllRes; j++ )
			{
				...
				{
					m_iSpriteCount++;
					//gEngfuncs.pfnConsolePrint(va("Sprite accepted with res of: %d\n", p->iRes));
				}
				p++;
			}
	...
	m_HUD_number_0 = GetSpriteIndex( "Hud2_Num0" );
	m_HUD_amnumber_0 = GetSpriteIndex( "Hud2_ANum0" );

	plushairindex = gHUD.GetSpriteIndex( "crosshairplus" );
