...
extern void EV_HLDM_SparkEffect( float *pos );
extern cvar_t* cl_flashlight_style;
...
//	RecClProcessPlayerState(dst, src);
	...
	// New flashlight code... Basically, you can either have the HL1
	// flashlight, the alpha one, or both!!!! Sounds fun right?
	// - serecky 1.2.26

	dlight_t* new_dl = NULL;
	dlight_t* dl = NULL;

	if (src->effects & EF_DIMLIGHT_2)
	{
		if (!cl_flashlight_style->value || cl_flashlight_style->value > 1)
		{
			new_dl = gEngfuncs.pEfxAPI->CL_AllocDlight(2);
			...
			VectorCopy(src->origin, new_dl->origin);
			VectorAdd(new_dl->origin, view_ofs, new_dl->origin);

			VectorMA(new_dl->origin, FLASHLIGHT_DISTANCE, forward, end);
			...
			gEngfuncs.pEventAPI->EV_SetSolidPlayers ( dst->number - 1 );	

			gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );

			gEngfuncs.pEventAPI->EV_PlayerTrace( new_dl->origin, end, PM_STUDIO_BOX, -1, &tr );
			...
			VectorCopy(tr.endpos, new_dl->origin);
			...
			new_dl->radius = 80;
			new_dl->color.r = new_dl->color.g = new_dl->color.b = 255 * falloff;
			new_dl->die = gEngfuncs.GetClientTime() + 0.2f;
			new_dl->decay = 2048;

			//gEngfuncs.pfnConsolePrint("Drawing regular flashlight...\n");

			gEngfuncs.pEventAPI->EV_PopPMStates();
		}

		if (cl_flashlight_style->value)
		{
			...
			//gEngfuncs.pfnConsolePrint("Drawing alpha flashlight...\n");
		}
	}
}
...
//	TODO: make this a cvar
	int iMuzzleFlash = 0, iSparkEffect = 1;
	...
	case 5002:
		if ( iSparkEffect )
			EV_HLDM_SparkEffect( (float *)&entity->attachment[0] );
		break;
	// Client side sound
	case 5004:		
...
// Experiment
			...
			if ( pTemp->flags & FTENT_HORNETTRAIL )
			{
				gEngfuncs.pEfxAPI->R_RocketTrail (pTemp->entity.prevstate.origin, pTemp->entity.origin, 6);
			}
