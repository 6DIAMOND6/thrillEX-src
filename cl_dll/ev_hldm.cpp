...
extern "C"
{
	...
	void EV_LadderClimb( struct event_args_s *args );
...
char *EV_HLDM_DamageDecal( physent_t *pe )
{
	...
	if ( pe->classnumber == 1 || pe->rendermode != kRenderNormal )
	{
...
void EV_HLDM_DecalGunshot( pmtrace_t *pTrace, vec3_t org, int iBulletType )
{
	physent_t *pe;
...
int EV_HLDM_CheckTracer( int idx, float *vecSrc, float *end, float *forward, float *right, int iBulletType, int iTracerFreq, int *tracerCount )
{
	int tracer = 0;
	int i;
	qboolean player = idx >= 1 && idx <= gEngfuncs.GetMaxClients() ? true : false, shouldTrace;

	if ( iTracerFreq == TRACER_ALWAYS )
		shouldTrace = true;
	else
		shouldTrace = iTracerFreq != TRACER_NEVER && ( (*tracerCount)++ % iTracerFreq) == 0;	// either no tracer or only every second shot
	...
	if ( shouldTrace )
	{
		...
		if ( iTracerFreq != TRACER_ALWAYS )		// guns that always trace also always decal
			tracer = 1;
...
void EV_HLDM_FireBullets( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float flDistance, int iBulletType, int iTracerFreq, int *tracerCount, float flSpreadX, float flSpreadY )
{
	...
	for ( iShot = 1; iShot <= cShots; iShot++ )	
	{
		...
		if ( tr.fraction != 1.0 )
		{
			...
			for ( i = 0; i < 3; i++ )
				vecOrg[i] = tr.endpos[i] - (vecDirShooting[i] * 4.0f);
			...
			case BULLET_PLAYER_MP5:	
				...
				EV_HLDM_DecalGunshot( &tr, vecOrg, iBulletType );
				break;
			case BULLET_PLAYER_BUCKSHOT:
				EV_HLDM_DecalGunshot( &tr, vecOrg, iBulletType );
				break;
...
void EV_FireGlock1( event_args_t *args )
{
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_9MM, TRACER_NEVER, 0, args->fparam1, args->fparam2 );
}
...
void EV_FireGlock2( event_args_t *args )
{
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_9MM, TRACER_NEVER, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireShotGunDouble( event_args_t *args )
{
	...
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, TRACER_NEVER, &tracerCount[idx-1], 0.17365, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, TRACER_NEVER, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
...
void EV_FireShotGunSingle( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 4, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, TRACER_NEVER, &tracerCount[idx-1], 0.08716, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, TRACER_NEVER, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
//======================
//	   SHOTGUN END
...
void EV_FireMP5( event_args_t *args )
{
	...
	static float flNextAnimTime = 0.0f;
	float flDeltaTime = flNextAnimTime - gEngfuncs.GetClientTime();
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		// Added this here for those who for some reason like
		// Half-Life alpha's weird MP5 animation skipping...
		// - serecky 1.3.26
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "hw_gun3.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, TRACER_SOMETIMES, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, TRACER_ALWAYS, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
}
...
void EV_FirePython( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		{
			EV_MuzzleFlash();
			gEngfuncs.pEventAPI->EV_WeaponAnimation( 8, 0 );
			V_PunchAxis( 0, -10.0 );
		}
		else
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation( 4, 0 );
		}
	}
	...
	if (args->iparam1)
	{
		...
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_357, TRACER_NEVER, 0, args->fparam1, args->fparam2 );
	}
}
//======================
//	    PHYTON END 
	...
	if ( tr.fraction < 1.0 )
	{
		...
		else if ( pe->rendermode == kRenderNormal ) 
		{
			...
			if ( gEngfuncs.PM_PointContents( tr.endpos, NULL ) != CONTENTS_WATER)
			{
				...
				myPos = tr.endpos - forward * 10;
				myPos = myPos + right * 6;
				myPos = myPos - up * 4;

				EV_HLDM_SparkEffect( myPos );
			}
...
int g_switchAnims[] = { EGON_ALTFIREON, EGON_ALTFIREOFF };
...
void EV_EgonFire( event_args_t *args )
{
	...
	//Only play the weapon anims if I shot it.
	if ( EV_IsLocal( idx ) && iFireMode == FIRE_NARROW )
...
void EV_EgonSwitch( event_args_t *args )
{
	int idx;
	...
	if ( iFireMode == FIRE_NARROW )
		gEngfuncs.pEventAPI->EV_WeaponAnimation ( g_switchAnims[ 0 ], 2 );
	else
		gEngfuncs.pEventAPI->EV_WeaponAnimation ( g_switchAnims[ 1 ], 2 );
}
//======================
//	    EGON END 
//======================
...
// SERECKY JAN-17-26: For some reason there's nothing in here that checks
// whether or not we can play the throwing animation, so I just slapped
// the animation checking code onto here.

void EV_ChubFire( event_args_t *args )
{
	int idx;
	vec3_t origin, angles, vecSrc, forward;
	pmtrace_t tr;
	...
	// Grab predicted result for local player
	gEngfuncs.pEventAPI->EV_LocalPlayerViewheight( vecSrc );

	// pull out of the player bounding box
	VectorMA(vecSrc, VEC_HUMAN_HULL_MAX[0], forward, vecSrc);
	...
	gEngfuncs.pEventAPI->EV_SetSolidPlayers ( idx - 1 );	
	gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );
	gEngfuncs.pEventAPI->EV_PlayerTrace( vecSrc + forward * 24, vecSrc + forward * 64, PM_NORMAL, -1, &tr );

	if ( tr.allsolid == 0 && tr.startsolid == 0 && tr.fraction > 0.25 )
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation ( CHUB_THROW, 0 );
		gEngfuncs.Con_Printf( "animating chub\n" );
	}
...
// Ended up not using this in the code because I could just
// use physkeys inside of PM_Shared.c instead... - serecky 1.14.26

void EV_LadderClimb( event_args_t *args )
{
	int idx;
	vec3_t origin;
	...
	if ( EV_IsLocal ( idx ) )
	{
		if (ladderpunch)
		{
			V_PunchAxis( 0, CLIMB_PUNCH_X );
			V_PunchAxis( 2, CLIMB_PUNCH_Z );
		}
		else
		{
			V_PunchAxis( 0, CLIMB_PUNCH_X );
		...
		ladderpunch = (ladderpunch + 1) % 2;
	}
	
	switch ( gEngfuncs.pfnRandomLong ( 0 , 3 ) )
	{
		case 0:	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_VOICE, "player/pl_pain2.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_VOICE, "player/pl_pain4.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM );	break;
		case 2:	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_VOICE, "player/pl_pain5.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_VOICE, "player/pl_pain6.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM );	break;
	}
}	
...
#define	SPARK_FRAMERATE		25.0
#define	SPARK_RENDERAMT		128

void SparkEffectThink ( struct tempent_s *ent, float frametime, float currenttime )
{
	if (currenttime < ent->entity.baseline.fuser1)
		return;

	ent->entity.curstate.framerate = SPARK_FRAMERATE;
	ent->entity.curstate.renderamt = gEngfuncs.pfnRandomLong(SPARK_RENDERAMT - 32, SPARK_RENDERAMT); // make it random so series of sparks wouldnt look so ugly
}

void EV_HLDM_SparkEffect( float *pos )
{
	TEMPENTITY* pSpark = NULL;
	Vector vecOrigin;
	int cFlag, iModel;

	// UNDONE: add tracecheck so sprites wont appear inside walls?
	vecOrigin[0] = pos[0];
	vecOrigin[1] = pos[1];
	vecOrigin[2] = pos[2];

	cFlag = FTENT_SPRANIMATE | FTENT_CLIENTCUSTOM;

	iModel = gEngfuncs.pEventAPI->EV_FindModelIndex( "sprites/spark1.spr" );

	pSpark = gEngfuncs.pEfxAPI->R_TempSprite( vecOrigin, vec3_origin, gEngfuncs.pfnRandomFloat(0.9, 1.0), iModel, kRenderTransAdd, kRenderFxNone, 255.0, 0, cFlag );

	if ( !pSpark )
		return;

	pSpark->callback = &SparkEffectThink;
	pSpark->entity.baseline.fuser1 = gEngfuncs.GetClientTime();
}
