...
#define RGB_GREENISH 0x00057D05	//5,125,5
...
public:
	...
	int DrawAlphaWList(float flTime); // NEW - serecky 1-1-26
	...
	int MsgFunc_RadPickup( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_AntPickup( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_AdrPickup( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_Airtank( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_LonJumBat( const char *pszName, int iSize, void *pbuf );
	void DrawCrosshairScalable(HSPRITE spr, wrect_t prc, int r, int g, int b);
	...
	WEAPON *m_pWeapon;
	cvar_t *m_pCvarWListStyle; // NEW - serecky 1-1-26
private:
	float m_fFade;
	RGBA  m_rgba;
	int	m_HUD_bucket0;
	...
	int m_radiationCount;
	int m_antidoteCount;
	int m_adrenalineCount;
	int m_iAirtank;
	int m_iLongJumpBat;

	wrect_t *m_prcSmallNum;
};
...
public:
	...
	int		m_iCoord[2];
private:
	HSPRITE m_hSpriteOn[2];
	HSPRITE m_hSpriteOff[2];
	wrect_t	*m_prcOff;
	wrect_t	*m_prcOn;
	float	m_flBat;	
	int		m_iBat;	
...
public:
	...
	cvar_t	*m_pCvarDraw;
	cvar_t	*m_pCvarScale;		// Tri_API HUD
	cvar_t	*m_pCvarCrosshair;
	cvar_t	*m_pCvarApparatus;
	cvar_t	*m_pCvarFovAdjust;

	// Tri_API HUD
	int		m_iHudScaleWidth;
	int		m_iHudScaleHeight;
	int		m_iViewsize;

	int m_iFontHeight;
	int DrawHudNumber(int x, int y, int iFlags, int iNumber, int r, int g, int b, int a, int iSprite = 0 );
...
//	CHudBattery		m_Battery;
	...
	int _cdecl MsgFunc_RateOfFire(const char *pszName,  int iSize, void *pbuf);
