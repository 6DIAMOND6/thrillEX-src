...
// John Barnold
//=========================================================
// UNDONE: Add tracehull for special death animations!!!
...
enum
{
	SCHED_HOLSTER_WEAPON = LAST_TALKMONSTER_SCHEDULE + 1,
};
...
Task_t	tlBaStopFollowing[] =
{
	{ TASK_CANT_FOLLOW,		(float)0 },
};

Schedule_t	slBaStopFollowing[] =
{
	{
		tlBaStopFollowing,
		ARRAYSIZE ( tlBaStopFollowing ),
		0,
		0,
		"Barney StopFollowing"
	},
};
...
//=========================================================
// IRelationship - overriden for Barney so we can make 
// him player's enemy on specific spawnflag.
//=========================================================
int CBarney::IRelationship( CBaseEntity *pTarget )
{
	if ( FClassnameIs ( pTarget->pev, "monster_barney_evil" ) )
		return R_DL;	// quick hack to make talkmonsters shoot or run from the evil barney
...
//	UTIL_SetSize(pev, Vector( -18, -18, 0 ), Vector( 18, 18, 72 )  );	// 0.52 values
	...
	m_flDistDieForward	= 80;
	m_flDistDieBackward	= 96;

	MonsterInit();

//	if ( !FBitSet(pev->spawnflags, SF_BARNEY_EVIL) )
...
void CBarney :: Precache()
{
	...
	PRECACHE_MODEL("models/barney_traitor.mdl");
...
void CBarney :: TalkInit()
{
	...
	m_szGrp[TLK_PLDIE1] =	"!BA_PLDIE";
	m_szGrp[TLK_PLDIE2] =	"!BA_PLDIE";
...
// anim to play.
//=========================================================
Activity CBarney :: GetDeathActivity ( void )
{
	TraceResult	tr;

	if ( CBaseMonster :: GetDeathActivity() == ACT_DIEVIOLENT )
	{
		UTIL_TraceHull ( Center(), Center() - gpGlobals->v_forward * 208, dont_ignore_monsters, head_hull, edict(), &tr );

		if ( tr.flFraction != 1.0 )
		{
			return ACT_DIESIMPLE;
		}
		else return ACT_DIEVIOLENT;
	}

	return CBaseMonster :: GetDeathActivity();
}
...
Schedule_t* CBarney :: GetScheduleOfType ( int Type )
{
	...
	case SCHED_HOLSTER_WEAPON:
			return slBarneyHolster;
		break;

	case SCHED_CANT_FOLLOW:
		return slBaStopFollowing;
...
// dead enemy
			...
			if (m_fGunDrawn && HasConditions ( bits_COND_NO_AMMO_LOADED ) )
	...
	case MONSTERSTATE_IDLE:
		{
			...
			if ( m_fGunDrawn )
			{
			//	ALERT(at_console, "Barney: Holster\n");
				return GetScheduleOfType( SCHED_HOLSTER_WEAPON );
			}
...
void CDeadBarney::BecomeDead( void )
{
...
class CEvilBarney : public CBarney
{
public:
	void Spawn( void );
	int  IRelationship( CBaseEntity *pTarget );
	int  FOkToSpeak( void ) { return FALSE; }
	int TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType) 
	{return CBaseMonster :: TakeDamage( pevInflictor, pevAttacker, flDamage, bitsDamageType);}
	void Killed( entvars_t *pevAttacker, int iGib );

	void TalkInit( void ) {CTalkMonster::TalkInit();};
	void PainSound(void);
	void AlertSound(void);

};
LINK_ENTITY_TO_CLASS( monster_barney_evil, CEvilBarney );

void CEvilBarney :: Spawn()
{
	CBarney :: Spawn();

	SET_MODEL(ENT(pev), "models/barney_traitor.mdl");
	UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
...
// him player's enemy on specific spawnflag.
//=========================================================
int CEvilBarney::IRelationship( CBaseEntity *pTarget )
{
	if ( pTarget->Classify() != CLASS_NONE )
		return R_DL;	// i hate everyone
	return CTalkMonster::IRelationship( pTarget );
}
...
void CEvilBarney :: PainSound ( void )
{
...
void CEvilBarney :: AlertSound( void )
{
	EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "barney/ba_attack1.wav", 1, ATTN_NORM, 0, GetVoicePitch());
}
...
//=========================================================
// Traitor Barney - a guard that collaborates with the military personnel
//=========================================================
class CTraitorBarney : public CEvilBarney
{
public:
	int  Classify ( void ){ return CLASS_HUMAN_MILITARY; }
	int  IRelationship( CBaseEntity *pTarget ){ return CBarney::IRelationship(pTarget); }
};
LINK_ENTITY_TO_CLASS( monster_barney_traitor, CTraitorBarney );