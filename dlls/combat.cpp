...
extern cvar_t knockback; // NEW - serecky 1.4.26
...
void CGib :: SpawnStickyGibs( entvars_t *pevVictim, Vector vecOrigin, int cGibs, int gibType )
{
	int i;
...
*/
			...
			case STICKYGIB_PINK:
			pGib->Spawn( "models/stickygibpink.mdl" );
			pGib->pev->body = RANDOM_LONG(0,2);

//			pGib->pev->origin.x = vecOrigin.x + RANDOM_FLOAT( -.5, .5 );
//			pGib->pev->origin.y = vecOrigin.y + RANDOM_FLOAT( -.5, .5 );
//			pGib->pev->origin.z = vecOrigin.z + 2;

			pGib->pev->origin.x = pevVictim->absmin.x + pevVictim->size.x * (RANDOM_FLOAT ( 0 , 1 ) );
			pGib->pev->origin.y = pevVictim->absmin.y + pevVictim->size.y * (RANDOM_FLOAT ( 0 , 1 ) );
			pGib->pev->origin.z = pevVictim->absmin.z + ( pevVictim->size.z - .5 );
			break;
			}
...
// UNDONE: replace this hardcoded nonsense with custom set of gibs for each monster
void CGib :: SpawnSpecialGibs( entvars_t *pevVictim, int gibType )
{
	CGib *pGib = GetClassPtr( (CGib *)NULL );
	...
	switch(gibType)
	{
	case SPECIALGIB_AGRUNT:
		pGib->Spawn( "models/AGRUNT_Gibs1.mdl" );
		break;
	case SPECIALGIB_HGRUNT:
		pGib->Spawn( "models/gib_hgrunt.mdl" );
		pGib->pev->skin = pevVictim->skin;
		break;
	case SPECIALGIB_HOUNDEYE:
		pGib->Spawn( "models/hegib.mdl" );
		break;
	case SPECIALGIB_ZOMBIE:
		pGib->Spawn( "models/zombiegibs1.mdl" );
		break;
	case SPECIALGIB_ISLAVE:
		pGib->Spawn( "models/islave_gibs1.mdl" );
		break;
	case SPECIALGIB_NONE:
	default:
		return;
		break;
	}
...
int	CBaseMonster::MainGib( void )
{
	if (HasHumanGibs())
		return MAINGIB_HUMAN;
	
	if (HasAlienGibs())
		return MAINGIB_ALIEN;

	return MAINGIB_NONE;
}

int	CBaseMonster::StickyGib( void )
{
	if (HasHumanGibs())
		return STICKYGIB_NORMAL;

	return STICKYGIB_NONE;
}
...
// TODO: custom gibs with char gib functions?
...
void CBaseMonster :: GibMonster( void )
{
	...
	if ( MainGib() > MAINGIB_NONE )
		CGib::SpawnRandomGibs( pev, 4, MainGib() );

	if ( SpecialGib() > SPECIALGIB_NONE )
		CGib::SpawnSpecialGibs( pev, SpecialGib() );
	
	if ( StickyGib() > STICKYGIB_NONE )
		CGib::SpawnStickyGibs( pev, pev->origin, 4, StickyGib() );

	if ( HasHumanGibs() )
		CGib::SpawnHeadGib( pev );

	if ( MainGib() + SpecialGib() + StickyGib() > 0 )
		gibbed = TRUE;
...
Activity CBaseMonster :: GetDeathActivity ( void )
{
	...
	if ( !m_flDistDieForward )
		m_flDistDieForward	= 32;

	if ( !m_flDistDieBackward )
		m_flDistDieBackward	= 32;
	...
	if ( deathActivity == ACT_DIEFORWARD )
	{
			...
			UTIL_TraceHull ( vecSrc, vecSrc + gpGlobals->v_forward * m_flDistDieForward, dont_ignore_monsters, head_hull, edict(), &tr );
	...
	if ( deathActivity == ACT_DIEBACKWARD )
	{
			...
			UTIL_TraceHull ( vecSrc, vecSrc - gpGlobals->v_forward * m_flDistDieBackward, dont_ignore_monsters, head_hull, edict(), &tr );
	...
	if ( deathActivity == ACT_DIEFORWARD || deathActivity == ACT_DIEBACKWARD || !ShouldThrowCorpse() )
		m_fThrowCorpse = FALSE;
	else
		m_fThrowCorpse = TRUE;
...
{
//	ALERT ( at_console, "CBaseMonster: BecomeDead!\n");
...
{
//	ALERT ( at_console, "CBaseMonster: ShouldGibMonster!\n");
...
//	if ( Classify() )
//		EMIT_SOUND(ENT(pev), CHAN_STATIC, "common/adios.wav", 1, ATTN_NORM);
...
void CBaseMonster :: SUB_StartFadeIn ( void )
{
	pev->rendermode = kRenderTransTexture;
	pev->renderamt = 0;
	...
	pev->nextthink = gpGlobals->time;
//	SetThink ( &CBaseMonster::SUB_FadeIn );
	SetThink ( &CBaseMonster::CallFadeIn );
}

void CBaseMonster :: SUB_FadeIn ( void  )
{
	if ( pev->renderamt < 250 && !m_skipPortal )
	{
		pev->renderamt += 12;
		pev->nextthink = gpGlobals->time + 0.05;
	}
	else 
	{
		...
		// Telefrag those who try to clip into us
		Create( "teledeath", pev->origin, pev->angles, edict() );
	}
}
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( m_MonsterState == MONSTERSTATE_SCRIPT && FBitSet(pev->spawnflags, SF_MONSTER_WAIT_FOR_SCRIPT) )
		return 0;
	...
	if ( (pev->flags & FL_MONSTER) && !FNullEnt(pevAttacker) )
	{
		...
		{// only if the attack was a monster or client!
			...
			if ( IsLightDamage(flDamage) )
			{
				SetConditions(bits_COND_LIGHT_DAMAGE);
			}

			if ( IsHeavyDamage(flDamage) )
			{
...
//	UTIL_DrawBeam( vecLookerOrigin, vecTargetOrigin, 128, 8, 8 );
...
void CBaseMonster :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if ( pev->takedamage )
	{
		...
		// SERECKY JAN-17-26: Added check for scripted things so they don't look funny...
		...
		{ 
				...
				&& pev->movetype != MOVETYPE_NONE && pev->movetype != MOVETYPE_FLY)
			{
				...
				pev->origin.z += 1.0f;
				pev->velocity.x += vecDir.x * (gMultiDamage.amount * 2.5f);
				pev->velocity.y += vecDir.y * (gMultiDamage.amount * 2.5f);
				pev->velocity.z += gMultiDamage.amount / 2.0f;
			}
		}
		else
		{
			SpawnBlood( vecOrigin, BloodColor(), flDamage );// a little surface blood.
		}
...
================
*/
// UNDONE: iTracerFreq does nothing and its presence is confusing, remove it
// SERECKY JAN-15-26: Added iForceImpact for server-sided Quake styled bullet impacts.
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand, int iForceImpact )
{
	static int tracerCount;
	TraceResult tr;
	Vector vecRight = gpGlobals->v_right;
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			{
				Vector vecNewOrg;
				vecNewOrg = tr.vecEndPos - (vecDir * 8.0f);
				DecalGunshot2( &tr, iBulletType, vecNewOrg );
			}
		...
		//	ALERT(at_console, "%d\n", iForceImpact);
			...
			case BULLET_PLAYER_BUCKSHOT:	
				 // make distance based!
				int m_bitsBuckshotDamage;
				m_bitsBuckshotDamage &= DMG_BULLET;
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgBuckshot, vecDir, &tr, m_bitsBuckshotDamage); 
				break;
