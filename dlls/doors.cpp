...
TYPEDESCRIPTION	CBaseDoor::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CBaseDoor, m_fPsychoActivated, FIELD_BOOLEAN ),
};
...
void PlayLockSounds(entvars_t *pev, locksound_t *pls, int flocked, int fbutton, CBaseEntity* pOther)
{
	// LOCKED SOUND
	...
	if (flocked)
	{
		...
		int fprintmessage = (pls->flwaitLockedMessage && gpGlobals->time > pls->flwaitLockedMessage);
		float fvol;
		...
		// SERECKY JAN-15-26: Doors now have locked messages.
		if ( !FStringNull( pev->message ) && (!pls->flwaitLockedMessage || (gpGlobals->time > pls->flwaitLockedMessage)) )
		{
			UTIL_ClientPrintAll(HUD_PRINTCENTER, STRING(pev->message));
			//DarkKrysteq:Doors play a sound anyway so don't play custom one.
			pls->flwaitLockedMessage = gpGlobals->time + 2.0f;
		}
...
// set the door's "in-motion" sound
	...
	case	11:
		...
		pev->noiseMoving = MAKE_STRING("plats/platmove1.wav");
		break;
	case	12:
		PRECACHE_SOUND ("plats/train2.wav");
		pev->noiseMoving = MAKE_STRING("plats/train2.wav");
		break;
	default:
		pev->noiseMoving = ALLOC_STRING("common/null.wav");
		break;
...
// set the door's 'reached destination' stop sound
	...
	case	10:
		...
		pev->noiseArrived = MAKE_STRING("plats/train1.wav");
		break;
	default:
		pev->noiseArrived = ALLOC_STRING("common/null.wav");
		break;
...
void CBaseDoor::DoorTouch( CBaseEntity *pOther )
{
	...
	// Psycho doors do not react on being touched
	if (FBitSet(pev->spawnflags, SF_DOOR_PSYCHO) )
		return;
	...
	if (m_sMaster && !UTIL_IsMasterTriggered(m_sMaster, pOther))
		PlayLockSounds(pev, &m_ls, TRUE, FALSE, pOther);
...
//	m_hActivator = pActivator;
	...
	if ( FBitSet(pev->spawnflags, SF_DOOR_PSYCHO) )
	{
		m_fPsychoActivated = !m_fPsychoActivated;
		DoorActivate();	
		return;
	}
...
int CBaseDoor::DoorActivate( )
{
	...
	if ( FBitSet(pev->spawnflags, SF_DOOR_PSYCHO) && m_fPsychoActivated )
	{
		if ( m_toggle_state != TS_AT_BOTTOM )
			DoorGoDown();
	...
	{// door should open
		...
		if ( m_hActivator != NULL && m_hActivator->IsPlayer() && m_bHealthValue != 0 )
		{// give health if player opened the door (medikit)
		// VARS( m_eoActivator )->health += m_bHealthValue;
			
			if ( m_hActivator->pev->health < m_hActivator->pev->max_health )
			{
		...
		PlayLockSounds(pev, &m_ls, FALSE, FALSE, NULL);
...
void CBaseDoor::DoorHitTop( void )
{
	...
	else if ( FBitSet(pev->spawnflags, SF_DOOR_PSYCHO) && m_fPsychoActivated || !FBitSet(pev->spawnflags, SF_DOOR_PSYCHO) )
	{
...
//	if ( FClassnameIs( pOther->pev, "monster_tentacle" ) )	// quick hack to avoid doors not opening in the silo of death
//		return;
...
public:
	...
	void EXPORT StopMoveSound();	// SamVanheer's fix https://github.com/ValveSoftware/halflife/issues/3265
