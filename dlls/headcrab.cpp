	...
	switch( pEvent->event )
	{
		case HC_AE_JUMPATTACK:
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				if (height < 16)
					height = 16;
				else if ( height > 120 ) // Vasia: Limit the height headcrab cam jump too. Taken from HL:Source
				{
					height = 120;
				}
...
void CHeadCrab :: LeapTouch ( CBaseEntity *pOther )
{
	...
	if ( !FBitSet( pev->flags, FL_ONGROUND ) )
	{
		...
		if ( g_iSkillLevel == SKILL_HARD && pOther->IsPlayer() && !(pOther->pev->flags & FL_GODMODE) && headcrab_dmg.value )	// only one tap player cuz there is no god
		{
			// Made SVAR that makes it so that headcrabs don't insta-kill on hard
			// if we have armor. Behaviour is adjustable with SVARS though.
			// - serecky 1.9.26

			if (headcrab_dmg.value == 2 && pOther->pev->armorvalue)
			{
