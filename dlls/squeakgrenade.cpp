...
#ifndef CLIENT_DLL
			CBaseEntity *pSqueak = CBaseMonster::Create( "monster_snark", tr.vecEndPos, m_pPlayer->pev->v_angle, m_pPlayer->edict() );
...
class CSqueakNest : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
	...
	int Classify( void );
	void SetYawSpeed( void ) { pev->yaw_speed = 60; }
...
LINK_ENTITY_TO_CLASS( monster_snarknest, CSqueakNest );

void CSqueakNest :: Spawn ( void )
{
	Precache( );
	...
	ResetSequenceInfo( );
	MonsterInit();
	SetUse(&CSqueakNest::PickupUse);
}

void CSqueakNest :: Precache ( void )
{
	PRECACHE_MODEL("models/w_sqknest.mdl");
	UTIL_PrecacheOther("monster_snark");
}

int CSqueakNest :: Classify ( void )
{
	return CLASS_ALIEN_BIOWEAPON;
}

int CSqueakNest::IRelationship( CBaseEntity *pTarget )
{
	if ( pTarget->Classify() == this->Classify() )
		return R_NO;

	return R_FR;
}

void CSqueakNest :: PickupUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
//	ALERT( at_console, "try to use shit\n");
	...
	// can I have this?
	if ( !g_pGameRules->CanHaveAmmo( pPlayer, "Snarks", SNARK_MAX_CARRY ) )
	{
	//	ALERT( at_console, "can't have shit\n");
		return;
	}
...
void CSqueakNest :: Killed( entvars_t *pevAttacker, int iGib )
{
	pev->solid = SOLID_NOT;
	pev->effects = EF_NODRAW;
	pev->deadflag = DEAD_DEAD;
	
	EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "squeek/sqk_blast1.wav", 0.7, ATTN_NORM, 0, 80 + RANDOM_LONG(0,39) );

	CBaseEntity *pOwner = CBaseEntity::Instance(pev->owner);
	if ( pOwner != NULL )
	{
	//	ALERT( at_console, "shit's dead\n");
		pOwner->DeathNotice( pev );
	}
...
//	CGib::SpawnRandomGibs( pev, 3, MainGib() );
	...
	RadiusDamage ( pev->origin + Vector( 0, 0, 24 ), pev, pev, 36, CLASS_ALIEN_BIOWEAPON, DMG_ACID);

	if (ShouldGibMonster( iGib ))	// snarks die with their nest
	{
		ALERT( at_console, "not gonna shoot snarks\n");
	...
	Vector vecDir;
	CBaseEntity *pSqueak = NULL;

	vecDir = pev->origin + Vector(0, 0, 16) + gpGlobals->v_forward * 16;

	pSqueak = CBaseMonster::Create( "monster_snark", vecDir, pev->v_angle, edict() );
	pSqueak->pev->velocity = vecDir * 4;

	vecDir = pev->origin + Vector(0, 0, 16) + gpGlobals->v_right * 16;
