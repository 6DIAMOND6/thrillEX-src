...
//	ALERT(at_console, "the multisource in question\n");

	if ( useType == USE_OFF )	
	{
	/*	if ( pActivator != NULL )
		{
			if ( FBitSet ( pActivator->pev->flags , FL_MONSTER ) )
				return;
		}*/

		// UNDONE: add a proper variable to govern IsTriggered work

		if ( m_iTotal != 0 )
			m_iTotal = 0; // set entity count to zero to pass IsTriggered check
		else
			m_iTotal = 999;
...
void CBaseButton:: ButtonTouch( CBaseEntity *pOther )
{
	...
	if (!UTIL_IsMasterTriggered(m_sMaster, pOther))
	{
		...
		PlayLockSounds(pev, &m_ls, TRUE, TRUE, pOther);
		return;
	}
...
void CBaseButton::ButtonActivate( )
{
	...
	if (!UTIL_IsMasterTriggered(m_sMaster, m_hActivator))
	{
		...
		PlayLockSounds(pev, &m_ls, TRUE, TRUE, NULL);
		return;
	}
	else
	{
		// button is unlocked, play unlocked sound
		PlayLockSounds(pev, &m_ls, FALSE, TRUE, NULL);
	}
...
void CMomentaryRotButton::Blocked( CBaseEntity *pOther )
{
	if ( !pev->spawnflags & SF_MOMENTARY_DOOR )
		return;
