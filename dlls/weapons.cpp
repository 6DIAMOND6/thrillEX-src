...
int IsOriginalROFOn(void)
{
	return original_rof.value;
}
...
int DamageDecal( CBaseEntity *pEntity, int bitsDamageType, int decalType )
{	
	switch ( decalType )
	{
	case GUNSHOT_SMALL:
...
/*int BigDamageDecal( CBaseEntity *pEntity, int bitsDamageType )
{
	if ( !pEntity )
		return (DECAL_BIGSHOT1 + RANDOM_LONG(0,4));
	
	return pEntity->BigDamageDecal( bitsDamageType );
}*/
...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_PLAYER_BUCKSHOT:
			UTIL_GunshotDecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, GUNSHOT_SMALL ) );
			break;
		default:
			// smoke and decal
			UTIL_GunshotDecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, GUNSHOT_SMALL ) );
		...
		case BULLET_MONSTER_12MM:
			// smoke and decal
			UTIL_GunshotDecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, GUNSHOT_BIG ) );
			break;
		case BULLET_PLAYER_CROWBAR:
			// wall decal
			UTIL_DecalTrace( pTrace, DamageDecal( pEntity, DMG_CLUB, GUNSHOT_SMALL ) );
			break;
		}
	}
}

void DecalGunshot2( TraceResult *pTrace, int iBulletType, Vector vecOrigin )
{
	// Is the entity valid
	if ( !UTIL_IsValidEntity( pTrace->pHit ) )
		return;
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_MONSTER_MP5:
		case BULLET_PLAYER_BUCKSHOT:
			UTIL_DecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, GUNSHOT_SMALL ) );
			break;
		default:
			// smoke and decal
			UTIL_DecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, GUNSHOT_SMALL ) );
		...
		case BULLET_MONSTER_12MM:
			// smoke and decal
			UTIL_DecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, GUNSHOT_BIG ) );
			break;
		case BULLET_PLAYER_CROWBAR:
			// wall decal
			UTIL_DecalTrace( pTrace, DamageDecal( pEntity, DMG_CLUB, GUNSHOT_SMALL ) );
...
#if !defined( OEM_BUILD ) && !defined( HLDEMO_BUILD )
	// chub
	UTIL_PrecacheOtherWeapon( "weapon_chub" );
#endif
...
//	g_sModelIndexFireball = PRECACHE_MODEL ("sprites/zerogxplode.spr");// fireball
	g_sModelIndexFireball = PRECACHE_MODEL ("sprites/explode1.spr");
...
void CBasePlayerItem :: FallInit( void )
{
	...
	pev->solid = SOLID_TRIGGER; // SOLID_BBOX; 
								// why it was like that in a first place?
...
const char *CBasePlayerItem::pPickupSounds[] = 
{
...
/*	TraceResult tr;
	...
	UTIL_TraceLine( pev->origin, pOther->Center(), dont_ignore_monsters, edict(), &tr );
	...
	if (pOther->AddPlayerItem( this ))
	{
		...
		EMIT_SOUND(ENT(pPlayer->pev), CHAN_ITEM, pPickupSounds[ RANDOM_LONG(0,ARRAYSIZE(pPickupSounds)-1) ], 1, ATTN_NORM);
	}
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	if (m_pPlayer->m_inHolster && gpGlobals->time >= m_pPlayer->m_fHolsterWaitTime)
	{
		m_pPlayer->m_inHolster = FALSE;
	}
}
...
BOOL CBasePlayerWeapon::DefaultHolster( int iAnim, int skiplocal = 0, int body = 0, float holsterDelay = 0.0 )
{
	if (!CanHolster( ))
	{
	return FALSE;
	ALERT( at_console, "Can't holster now! \n");
	}
	else
	{
	return TRUE;
	...
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + holsterDelay + 1.0;

	m_pPlayer->m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + holsterDelay + 5.0;

	SendWeaponAnim( iAnim );
}

//=========================================================
// DrawerThink - fake collision function to avoid items 
// falling through BSP geometry in tight spots and 
// still be able to be picked up on a sensible distance
//=========================================================
void CBasePlayerAmmo::DrawerThink( void )
{
	CBaseEntity* pEntity = NULL;
	...
	if ( pEntity->IsPlayer() && AddAmmo(pEntity) )
	{
		SetThink(NULL);
		UTIL_Remove(this);
		return;
...
void CBasePlayerAmmo::Spawn( void )
{
	...
	if ( FBitSet( pev->spawnflags, SF_AMMO_INDRAWER ) )
	{
		...
		SetThink( &CBasePlayerAmmo::DrawerThink );
		pev->nextthink = gpGlobals->time + 0.1;
	}
	else UTIL_SetSize(pev, Vector(-16, -16, 0), Vector(16, 16, 16));
...
void CBasePlayerAmmo :: DefaultTouch( CBaseEntity *pOther )
{
	...
	if (AddAmmo( pOther ))
	{
		...
		else
		{
			...
			pev->nextthink = gpGlobals->time + .1;
			EMIT_SOUND(ENT(pev), CHAN_ITEM, pAmmoSounds[ RANDOM_LONG(0,ARRAYSIZE(pAmmoSounds)-1) ], 1, ATTN_NORM);

			if ( !FStringNull (MAKE_STRING(GetAmmoSuitSentence())) )
			{
				CBasePlayer *pPlayer = (CBasePlayer*)Instance(pOther->edict());

				pPlayer->SetSuitUpdate( GetAmmoSuitSentence(), FALSE, SUIT_NEXT_IN_5MIN);
			}
...
{
	DEFINE_FIELD( CPython, m_flAttackDelay, FIELD_TIME ),
	DEFINE_FIELD( CPython, m_fSpotActive, FIELD_INTEGER ),	// it is important for python laserdot state to be saved for overall weapon's usability
};
