...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayer, m_pNextActiveItem, FIELD_CLASSPTR ),
...
int gmsgTeamNames = 0;
int gmsgRadiationCount = 0;
int gmsgAntidoteCount = 0;
...
void LinkUserMessages( void )
{
	...
	gmsgRadiationCount = REG_USER_MSG( "RadPickup", 2 );
	gmsgAntidoteCount = REG_USER_MSG( "AntPickup", 2 );
	gmsgAdrenalineCount = REG_USER_MSG( "AdrPickup", 2 );
	gmsgAirtank	= REG_USER_MSG("Airtank", 2);
...
// UNDONE: make an overload of this that would let us govern if there will be 
// suit update about healing or not
int CBasePlayer :: TakeHealth( float flHealth, int bitsDamageType )
{
	...
	if ( !(bitsDamageType & DMG_DROWN) )
		SetSuitUpdate("!HEV_HEAL8", FALSE, SUIT_NEXT_IN_5MIN);
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( m_rgItems[ITEM_SHIELD] )
		flBonus /= 2;
	
	// may be interrupted by other env_fades, needs to be replaced with sprite
	//UTIL_ScreenFade((CBaseEntity*)this,Vector(150,0,0),flDamage/16,1,100/*(int)flDamage*/,0);
	...
	// Armor. 
	if (pev->armorvalue && flDamage > 0 && !(bitsDamageType & (DMG_FALL | DMG_DROWN)) )// armor doesn't protect against fall or drown damage!
	{
		...
		if (flArmor > pev->armorvalue)
		{
			...
			{
			case 0:
				SetSuitUpdate("!HEV_E0", FALSE, SUIT_NEXT_IN_5MIN);
			break;
			
			case 1:
				SetSuitUpdate("!HEV_E1", FALSE, SUIT_NEXT_IN_5MIN);
			break;

			case 2:
				SetSuitUpdate("!HEV_E6", FALSE, SUIT_NEXT_IN_5MIN);
			break;
			}
		}
		else
	...
	if ( m_fCanRevive && m_flNextRevive )
	{
		BOOL fShouldIgnore = !(bitsDamage & DMG_FALL || bitsDamage & DMG_DROWN);

		// ignore all the damage below 15 except for drown and fall during revival
		if ( flDamage < PLAYER_REVIVED_HEALTH && fShouldIgnore )
			flDamage = 0;
	}
	...
	*/
	...
	if (fTookDamage && !ftrivial && fcritical && flHealthPrev < 75)
	{
		...
		{
			if ( RANDOM_LONG(0,1) )
				SetSuitUpdate("!HEV_HLTH3", FALSE, SUIT_NEXT_IN_5MIN);	// near death
			else
				SetSuitUpdate("!HEV_HLTH6", FALSE, SUIT_NEXT_IN_5MIN);
		}
		else if (pev->health < 20)
		{
			if ( RANDOM_LONG(0,1) )
				...
				SetSuitUpdate("!HEV_HLTH5", FALSE, SUIT_NEXT_IN_5MIN);
		}
...
{
	CSound *pSound;
	BOOL fGibbed = (( pev->health < -40 && iGib != GIB_NEVER ) || iGib == GIB_ALWAYS);
	BOOL fHadSuit = pev->weapons & (1<<WEAPON_SUIT);
	...
	// Revive player. Works only if player isn't gibbed.
	...
	else if ( (pev->weapons & (1<<WEAPON_SUIT)) && m_fCanRevive )
	{
		if ( m_rgItems[ITEM_ADRENALINE] <= 0 ) // do not revive if out of adrenaline
		{
			m_fCanRevive = FALSE;
		}
		else
...
void CBasePlayer::WaterMove()
{
	...
	if (pev->waterlevel != 3) 
	{
		...
		if ( m_iAirCapacity < 12 )
		{ 
		/*	if ( m_flNextBubble < gpGlobals->time )
			{
				m_iAirCapacity++;
				m_flNextBubble = gpGlobals->time + 1;
				ALERT(at_console, "m_iAirCapacity++\n");
				ALERT ( at_console, "Airtank capacity = %i\n", m_iAirCapacity );
			} 
		*/
			m_iAirCapacity = 12;
		}
	...
	{	// fully under water
		...
		if (pev->air_finished )
		{ 
			m_iAirCapacity = (int)(pev->air_finished - gpGlobals->time );
		//	ALERT ( at_console, "Airtank capacity = %i\n", m_iAirCapacity );
			/*if ( m_flNextBubble < gpGlobals->time )
			{
				m_iAirCapacity--;
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	{
//		ALERT(at_console, "wait for anim\n");
		StudioFrameAdvance( );
...
//	ALERT(at_console, "anim done\n");
	...
	if (pev->deadflag == DEAD_DEAD)
	{
		...
		else ALERT(at_console, "can't respawn\n");
...
Vector VecEntityCenter( entvars_t* pevObject )
{
	return (pevObject->absmax + pevObject->absmin) * 0.5;

}

void DrawSizeBox( entvars_t* pevObject, int r, int g, int b )
{
	if ( CVAR_GET_FLOAT("sv_drawroute") <= 0 )
		return;

	Vector vecPoint, vecOrigin = VecEntityCenter(pevObject);

	UTIL_ParticleEffect( vecOrigin, VecEntityCenter(pevObject), 79, 4 );

	UTIL_DrawBeam( pevObject->absmax, pevObject->absmin, 128, 4, 128 );
	return;

	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
			WRITE_BYTE( TE_BOX );
			WRITE_COORD( pevObject->mins.x );
			WRITE_COORD( pevObject->mins.y );
			WRITE_COORD( pevObject->mins.z );
			WRITE_COORD( pevObject->maxs.x );
			WRITE_COORD( pevObject->maxs.y );
			WRITE_COORD( pevObject->maxs.z );
...
void CBasePlayer::PlayerUse ( void )
{
	...
	while ((pObject = UTIL_FindEntityInSphere( pObject, pev->origin, PLAYER_SEARCH_RADIUS )) != NULL)
	{
		...
		if (pObject->ObjectCaps() & (FCAP_IMPULSE_USE | FCAP_CONTINUOUS_USE | FCAP_ONOFF_USE))
		{
			...
			if ( FClassnameIs(pObject->pev, "func_rot_button") || FClassnameIs(pObject->pev, "momentary_rot_button") )
				vecLOS = pObject->pev->origin - (pev->origin + pev->view_ofs);
			else
				vecLOS = (VecBModelOrigin( pObject->pev ) - (pev->origin + pev->view_ofs));
		//	UTIL_DrawBeam( pev->origin + pev->view_ofs, vecLOS, 224, 32, 4 );
...
//		if ( FClassnameIs(pObject->pev, "func_button") )
//			vecOrigin = VecBModelOrigin( pObject->pev );
//		else
//			vecOrigin = pObject->pev->origin; // UNDONE: this does not work correctly if origin isn't placed at the center of the model

		vecOrigin = VecEntityCenter( pObject->pev );

		DrawSizeBox ( pObject->pev, 128, 4, 4 );
			
		

		// This loop checks if the trace from player can reach at least one corner of the object's sizebox
		while ( i <= 8 )
		{
			...
			if ( i == 1 )
			{
				vecCheck.x += pObject->pev->size.x/2;
				vecCheck.y += pObject->pev->size.y/2;
				vecCheck.z += pObject->pev->size.z/2;
			}
			...
			UTIL_TraceLine(pev->origin + pev->view_ofs, vecCheck, ignore_monsters, edict(), &tr);

			UTIL_DrawBeam( pev->origin + pev->view_ofs, vecCheck, 128, 4, 4 );
			UTIL_DrawBeam( pev->origin + pev->view_ofs, tr.vecEndPos, 4, 128, 4 );

			if ( tr.pHit == pObject->edict() || tr.flFraction == 1.0 )
			{
			/*	if ( tr.pHit == pObject->edict() )
					ALERT(at_console, "phit\n");
				if ( tr.flFraction == 1.0 )
					ALERT(at_console, "fraction\n");*/
				canUse = TRUE;
				break;
			}
...
/*	switch( pmove->RandomLong(0,1) )
	{
	case 0:
	pmove->PM_PlaySound( CHAN_VOICE, "player/pl_jump1.wav", 1, ATTN_NORM, 0, PITCH_NORM );	
	case 1:
	pmove->PM_PlaySound( CHAN_VOICE, "player/pl_jump2.wav", 1, ATTN_NORM, 0, PITCH_NORM );
	}	*/
	...
	m_flJumpGroundDelay = gpGlobals->time + 0.025;
...
//	if ( m_rgbTimeBasedDamage[itbd_Paralyze] == 0 )  g_engfuncs.pfnSetClientMaxspeed(edict(), 0 );
	...
	// make sure that we are not waiting for player to revive
	// if we do, avoid starting PlayerDeathThink so player wont lose all of their items

	if (pev->deadflag >= DEAD_DYING )
	{
		if ( m_fCanRevive && m_rgItems[ITEM_ADRENALINE] )
		{
			if ( m_flNextRevive <= gpGlobals->time )
			{
				...
				pev->fov = m_iFOV;
				pev->view_ofs = VEC_VIEW;	// do not crawl

				if ( m_pActiveItem )
				m_pActiveItem->Deploy( );	// im back in action, give me my gun!
		...
		{
		//	ALERT( at_console, "PlayerDeathThink call\n" );
			PlayerDeathThink();
			return;
		}
	}
		...
		if (m_bitsDamageType & (DMG_PARALYZE << i))
		{
			...
			if (m_rgbTimeBasedDamage[i])
			{
				...
				if (((i == itbd_NerveGas) && (m_rgbTimeBasedDamage[i] < NERVEGAS_DURATION)) ||
					...
					((i == itbd_Paralyze)   && (m_rgbTimeBasedDamage[i] < PARALYZE_DURATION)))
				{
				...
				if ( ((i == itbd_Radiation) && (m_rgbTimeBasedDamage[i] < RADIATION_DURATION)) )
				{
					if (m_rgItems[ITEM_RADIATION])
					{
						m_rgbTimeBasedDamage[i] = 0;
...
/*	if ((FBitSet(pev->flags, FL_ONGROUND)) && ( m_flFallVelocity > 48 ) )
	{
		EMIT_SOUND(ENT(pev), CHAN_ITEM, "player/pl_jumpland2.wav", 1, ATTN_NORM);
		//UTIL_ScreenShake (pev->origin, 2.0, 0.5, 0.5, 0.0);
		pev->punchangle[ 0 ] = pmove->flFallVelocity * 0.013;	
	}	*/
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			{//splat
				...
				if ( !(pev->flags & FL_GODMODE) ) // don't deny adrenaline in godmode
					m_fCanRevive = FALSE;
			}
...
//	m_bitsDamageType = 0;	// ouch
	m_bitsDamageType &= DMG_TIMEBASED;
	m_bitsHUDDamage = -1;

	m_iClientBattery = -1;

	m_iTrain |= TRAIN_NEW;	// Vasia's fix
...
//	m_fHolsterWaitTime = gpGlobals->time + 1.0;
	m_fHolsterWaitTime = gpGlobals->time + m_pActiveItem->GetHolsterDelay();
...
int CBasePlayer::AddPlayerItem( CBasePlayerItem *pItem )
{
	...
	CBaseEntity *pOwner = CBaseEntity::Instance(pItem->pev->owner);
	if ( pOwner != NULL )
	{
		if ( FClassnameIs( pOwner->pev, "monstermaker") )
...
void CBasePlayer::ItemPreFrame()
{
	...
	/*	if ( g_pGameRules && g_pGameRules->IsMultiplayer() )
		{
			// quick fix for bots to properly work, come up with something else
			if ( pev->weapons & (1<<WEAPON_GLOCK) )
				SwitchWeapon( m_rgpPlayerItems[ WEAPON_GLOCK ]);
			else if ( pev->weapons & (1<<WEAPON_CROWBAR) )
				SwitchWeapon( m_rgpPlayerItems[ WEAPON_CROWBAR ]);
		}
	*/	
		return;
	}
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgRadiationCount, NULL, pev );
		WRITE_SHORT( m_rgItems[ITEM_RADIATION]);
		MESSAGE_END();

	MESSAGE_BEGIN( MSG_ONE, gmsgAntidoteCount, NULL, pev );
		WRITE_SHORT( m_rgItems[ITEM_ANTIDOTE]);
		MESSAGE_END();

	MESSAGE_BEGIN( MSG_ONE, gmsgAdrenalineCount, NULL, pev );
			WRITE_SHORT( m_rgItems[ITEM_ADRENALINE]);
			MESSAGE_END();	

	if ( gmsgAirtank )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgAirtank, NULL, pev );
			WRITE_SHORT(m_iAirCapacity);
		MESSAGE_END();
	}
...
void CDeadHEV :: Spawn( void )
{
	...
	pev->yaw_speed		= 8;
	pev->sequence		= 0;
//	pev->body			= 1;			// dead Gordon clones lying around just like in beta
	m_bloodColor		= BLOOD_COLOR_RED;
